...
respond = requests.post(
    url="http://127.0.0.1:8000/api/predict-mass",
    json=payload
)

# print(respond.json()["texts"])

for text, result in zip(respond.json()["texts"], respond.json()["is_positives"]):
    print(f"positif: {text}" if result else f"negatif: {text}")